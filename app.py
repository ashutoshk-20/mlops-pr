...
import numpy as np
MODEL_DIR = "models"
vectorizer = joblib.load(os.path.join(MODEL_DIR, "vectorizer_20250510_122512.joblib"))
nn_model = joblib.load(os.path.join(MODEL_DIR, "nn_model_20250510_122512.joblib"))
...
scaler_path = find_latest_file("scaler_*.joblib")
scaler = joblib.load(scaler_path)

# Load product IDs if available
try:
    product_ids = joblib.load(os.path.join(MODEL_DIR, "product_ids_20250510_122512.joblib"))
except Exception:
    product_ids = None

def parse_price(price_str):
    """Convert price string like '₹98,990' to float 98990.0"""
...
def get_rating(product):
    val = product.get('ratings', 0)
    if isinstance(val, dict):
        return float(val.get('average', 0))
    try:
        return float(val)
    except Exception:
        return 0.0

def get_review_length(product):
    # Example: count words in review or description
    review = product.get('review', '') or product.get('description', '')
    return len(str(review).split())

def extract_features(product):
    import numpy as np
    text = f"{product.get('titles', '')} {product.get('Brand', '')}"
    text_vec = vectorizer.transform([text]).toarray()[0]

    rating = get_rating(product)
    review_length = get_review_length(product)
    brand = str(product.get('Brand', '')).lower()
    is_apple = 1 if 'apple' in brand else 0
    is_samsung = 1 if 'samsung' in brand else 0
    is_redmi = 1 if 'redmi' in brand else 0
    is_realme = 1 if 'realme' in brand else 0
    is_oneplus = 1 if 'oneplus' in brand else 0
    ...
    numeric_vec = scaler.transform(numeric_vec)[0]
    full_vec = np.concatenate([text_vec, numeric_vec])
    return full_vec
...
def get_ml_recommendations(product_id, top_n=6):
    product = collection.find_one({"_id": ObjectId(product_id)})
    if not product:
        return []
    query_vec = extract_features(product).reshape(1, -1)
    distances, indices = nn_model.kneighbors(query_vec, n_neighbors=top_n+1)
    indices = indices[0][1:]
    recs = []
    for idx in indices:
        rec_id = product_ids[idx] if product_ids else None
        if rec_id:
            rec_product = collection.find_one({"_id": ObjectId(rec_id)})
        else:
            continue
        if rec_product:
            rec_product["_id"] = str(rec_product["_id"])
            rec_product["strategy"] = "ml_similar"
            recs.append(rec_product)
        if len(recs) >= top_n:
            break
    return recs
...
def product_detail(product_id):
    try:
        ...
        recommendations = get_ml_recommendations(product_id)
        if not recommendations:
            recommendations = get_recommendations(product_id)  # fallback
...
def chat():
    ...
    try:
        ...
        response = openai_client.chat.completions.create(
            model=OPENROUTER_MODEL, # Corrected variable name
            messages=messages,
            extra_headers={
